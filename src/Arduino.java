...
public class Arduino extends JVSMain implements MyOpenLabDriverOwnerIF
{
  ...
  private final VSInteger pollTime = new VSInteger(200);
  ...
  private final VSString Port=new VSString("/dev/ttyUSB1");
  private final VSBoolean taste1=new VSBoolean(false);
  private final VSBoolean taste2=new VSBoolean(false);
  private final VSBoolean taste3=new VSBoolean(false);
  private final VSBoolean taste4=new VSBoolean(false);
  private final VSBoolean taste5=new VSBoolean(false);
  private final VSBoolean taste6=new VSBoolean(false);

  private final VSInteger adc1=new VSInteger(0);
  private final VSInteger adc2=new VSInteger(0);
  private final VSInteger adc3=new VSInteger(0);
  private final VSInteger adc4=new VSInteger(0);
  private final VSInteger adc5=new VSInteger(0);
  private final VSInteger adc6=new VSInteger(0);
  ...
  public void init()
  {
    ...
    setPin(c++,ExternalIF.C_BOOLEAN,ExternalIF.PIN_OUTPUT);
    setPin(c++,ExternalIF.C_BOOLEAN,ExternalIF.PIN_OUTPUT);
    setPin(c++,ExternalIF.C_BOOLEAN,ExternalIF.PIN_OUTPUT); // Taste6

    setPin(c++,ExternalIF.C_INTEGER,ExternalIF.PIN_OUTPUT); // ADC-1
    setPin(c++,ExternalIF.C_INTEGER,ExternalIF.PIN_OUTPUT);
    setPin(c++,ExternalIF.C_INTEGER,ExternalIF.PIN_OUTPUT);
    setPin(c++,ExternalIF.C_INTEGER,ExternalIF.PIN_OUTPUT);
    setPin(c++,ExternalIF.C_INTEGER,ExternalIF.PIN_OUTPUT);
    setPin(c++,ExternalIF.C_INTEGER,ExternalIF.PIN_OUTPUT); // ADC-6
        
    setPin(c++,ExternalIF.C_BOOLEAN,ExternalIF.PIN_INPUT); // LED 1
  ...
  public void start()
  {
    ...
    timer = new javax.swing.Timer(pollTime.getValue(), (ActionEvent evt) -> {
        if (started && driver!=null)
        {
            byte value=0;
            
            if (led1.getValue() || firstUpdate) value|=(1); else value&=~(1);
